...
const Button: React.FC<ButtonProps> = ({
  ...
  const colorClasses = bordered
    ? "border border-current text-indigo-600 hover:bg-indigo-100 active:text-indigo-500"
  ...
  return (
    <Component
      href={href}
      className={`${baseClasses} ${colorClasses} ${className} transition-colors duration-300`}
      onClick={onClick}
    >
      <span>{label}</span>
    </Component>
